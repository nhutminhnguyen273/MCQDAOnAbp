...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 84827350-6e58-4a82-bf75-639ba682c5ee
```

> `84827350-6e58-4a82-bf75-639ba682c5ee` is the password of the certificate, you can change it to any password you want.
...
Run `MCQDAOnAbp.CourseService.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `MCQDAOnAbp.CourseService.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
